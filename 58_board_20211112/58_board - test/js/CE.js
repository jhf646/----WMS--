...
function load() {
    ...
    //5、 获取xx年12个月的出入库数量
    ajax("GetMonthDataByYear&year=2020")
        ...
        var option = {
            series: [{
                ...
                data: [{
                    ...
                    { value: data.warehouseOutCount, itemStyle: { color: '#12274d' } }, // 颜色#12274d
                    { value: data.warehouseInCount, itemStyle: { color: 'transparent' } } // 透明隐藏第三块区域
                ]
            }]
        };
...
function GetMonthDataByYear(data) {
    ...
    let arr = [],
        arr1 = []
    for (var i = 0; i < data.length; i++) {
        arr.append(data.InCount)
        arr1.append(data.OutCount)
    }
...
function GetUseRate(data) {
    ...
    let arr = [],
        arr1 = []
    for (var i = 0; i < data.length; i++) {
        arr.append(data.materielID)
        arr1.append(data.useRate)
    }
...
function GetFlangeStockInfo(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        $("#" + data[i].StockIndex + " #PlanNumber").html("")
        $("#" + data[i].StockIndex + " #PlanNumber").html(data[i].PlanNumber)
    }
}